// Package storetest hands integration tests a plain PostgreSQL 16 container
// with the engine migration chain applied. Every package that talks to the
// engine database tests against a real database rather than a mock, so they all
// start from this helper instead of repeating the container setup.
//
...
// The container the tests run against. The engine database is plain Postgres on
// the major version roadmap/00-conventions.md section 1 pins for it, without
// the TimescaleDB extension the reporting side needs.
const (
	image = "postgres:16"
...
// NewDB starts a Postgres container, waits until it accepts connections,
