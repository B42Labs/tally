...
// Finalize closes one billing period over a completed run of it, and is the
// whole of what tally-engine finalize does. The run's status and the period's
// move in one transaction, so no reader ever sees a period pointing at a run
// that is not finalized itself. Once that transaction commits, the D8 triggers
// of migration 0001 hold the run's usage records, rated records and statements
// against every write.
//
...
// ErrRunInProgress.
//
// The four refusals are a run no row carries (ErrRunNotFound), a run of another
// month (ErrPeriodMismatch), a run that is not completed (ErrRunNotCompleted),
// and a period that is already closed (ErrPeriodFinalized). None of them writes
// a row.
//
// The run's kind is deliberately not checked. A correction run is finalized the
// same way a regular one is, over this same function (roadmap WP 3.9).
func Finalize(ctx context.Context, engine *pgxpool.Pool, periodFrom time.Time, runID uuid.UUID) error {
	retry := fmt.Sprintf("tally-engine finalize --period %s --run %s", period.Format(periodFrom), runID)
	return withPeriodLock(ctx, engine, periodFrom, retry, func() error {
		return finalize(ctx, engine, periodFrom, runID)
	})
}

// finalize is Finalize under the period lock: the checks and the two writes, in
// one transaction. The run row is read FOR NO KEY UPDATE, so the status the
// checks are made against is the one the finalization writes over.
func finalize(ctx context.Context, engine *pgxpool.Pool, periodFrom time.Time, runID uuid.UUID) error {
